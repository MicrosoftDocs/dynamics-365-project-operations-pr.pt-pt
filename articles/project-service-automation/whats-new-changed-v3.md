...
1.  Como o **Recurso reservável**, que é um campo na entidade **Entrada de tempo**, não é exposto na página **Criação rápida**, é necessário adicioná-lo.
    ...
    Crie uma vista personalizada para visualizar apenas as entradas de tempo criadas para o recurso. Publique as personalizações no designer de módulo de aplicação para que esta visualização apareça sob **Seletor de vistas** na página **Entradas de tempo**. Existem dois plug-ins que tratam da definição do gestor para entradas de tempo que não são do projeto:
